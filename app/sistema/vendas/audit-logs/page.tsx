...
export default function AuditLogsPage() {
  ...
  const handleExportarCSV = () => {
    try {
      ...
      const csvContent = [
        ["Tabela", "ID do Registro", "Data da Deleção", "Deletado Por", "Dados"],
        ...logs.map((log) => [
          log.tabela_nome,
          log.registro_id || "N/A",
  ...
  const formatarDadosApagados = (tabela: string, dados: Record<string, any>) => {
    ...
    const formatarValor = (chave: string, valor: any): string => {
      ...
      // Valores monetários
      if (['valor_total', 'valor_pago', 'valor_desconto', 'saldo_devedor', 
           'preco_unitario', 'subtotal', 'valor'].includes(chave)) {
      ...
      // Datas
      if (chave.includes('data_') && typeof valor === 'string' && valor.includes('-')) {
                      ...
                      <TableRow key={log.id}>
                        ...
                        <TableCell>
                          <code className="text-xs bg-default-100 dark:bg-default-50 px-2 py-1 rounded text-foreground">
                            {log.registro_id ? log.registro_id.slice(0, 12) + "..." : "N/A"}
                          </code>
                        </TableCell>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="mt-2 space-y-2">
                        {formatarDadosApagados(logSelecionado.tabela_nome, logSelecionado.dados_apagados).map((campo, index) => (
                          <div key={index} className="flex justify-between p-2 bg-default-100 rounded">
                            <span className="text-sm font-medium text-foreground">{campo.nome}:</span>
                            <span className="text-sm text-foreground">{campo.valor}</span>
                          </div>
                        ))}
                      </div>
                      ...
                      <details className="mt-3">
                        ...
                        <div className="mt-2 p-3 bg-default-100 rounded max-h-64 overflow-auto">
                          <pre className="text-xs font-mono text-foreground">
                            {JSON.stringify(logSelecionado.dados_apagados, null, 2)}
                          </pre>
                        </div>
                      </details>
                    </div>
                    ...
                    {logSelecionado.motivo && (
                      <div>
                        ...
                        <p className="mt-1 text-foreground">{logSelecionado.motivo}</p>
                      </div>
                    )}
                  </div>
                )}
