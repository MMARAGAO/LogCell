...
export default function RelatorioLucroPage() {
  ...
  async function carregarDados() {
    ...
    try {
      ...
      const { data: aparelhos } = await query.order("data_venda", {
        ascending: false,
      });
      ...
      const vendaIds = Array.from(
        new Set(
          aparelhos.map((a: any) => a.venda_id).filter(Boolean) as string[],
        ),
      );
      ...
      brindesData.data?.forEach((b: any) => {
        brindesPV[b.venda_id] =
          (brindesPV[b.venda_id] || 0) + (b.valor_custo || 0);
      });

      const taxasPV: Record<string, number> = {};

      taxasData.data?.forEach((p: any) => {
        const t = (p.valor || 0) - (p.liquido || 0);

        if (t > 0) taxasPV[p.venda_id] = (taxasPV[p.venda_id] || 0) + t;
      });

      const vendedoresMap = new Map(
        vendedoresData.data?.map((v: any) => [v.id, v.nome]),
      );

      const lista = aparelhos.map((a: any) => {
        const custo =
          (a.valor_compra || 0) +
          (brindesPV[a.venda_id] || 0) +
          (taxasPV[a.venda_id] || 0);
  ...
  const porVendedor = useMemo(() => {
    const map = new Map<
      string,
      { bruto: number; lucro: number; qtd: number }
    >();

    dados.forEach((d) => {
      if (!map.has(d.vendedor_nome))
        map.set(d.vendedor_nome, { bruto: 0, lucro: 0, qtd: 0 });
  ...
  useEffect(() => {
    setPagina(1);
  }, [dataInicio, dataFim, lojaFiltro]);

  function exportarCSV() {
    const cabecalho =
      "Vendedor,Aparelho,IMEI,Data,Valor Bruto,Custo,Taxas,Brindes,Lucro";
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.06)] border border-gray-100 dark:border-zinc-800 p-5">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Aparelhos vendidos — lucro real por vendedor e período
                </p>
              </div>
            </div>
            <Button
          ...
          <div className="flex flex-wrap gap-3 mb-5">
            <Input
              classNames={{
                inputWrapper:
                  "bg-gray-50 dark:bg-zinc-800 border-gray-200 dark:border-zinc-700 w-40",
              }}
              label="Data Início"
              size="sm"
              type="date"
            ...
            <Select
              classNames={{
                trigger:
                  "bg-gray-50 dark:bg-zinc-800 border-gray-200 dark:border-zinc-700 w-48",
              }}
              label="Loja"
              selectedKeys={lojaFiltro ? [lojaFiltro] : []}
              size="sm"
              variant="bordered"
              onSelectionChange={(keys) =>
                setLojaFiltro((Array.from(keys)[0] as string) || "")
              }
            >
              {[{ id: 0, nome: "Todas" }, ...lojas].map((l: any) => (
                <SelectItem key={l.id === 0 ? "" : String(l.id)}>
                  {l.nome}
                </SelectItem>
              ))}
            </Select>
            <Select
              ...
              label="Vendedor"
              selectedKeys={vendedorFiltro ? [vendedorFiltro] : []}
              size="sm"
              variant="bordered"
              onSelectionChange={(keys) =>
                setVendedorFiltro((Array.from(keys)[0] as string) || "")
              }
            >
              {[{ id: "0", nome: "Todos" }, ...vendedores].map((v: any) => (
                <SelectItem key={v.id === "0" ? "" : v.id}>{v.nome}</SelectItem>
              ))}
            </Select>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            <KpiCard
              ...
              label="Valor Bruto"
              value={formatarMoeda(kpis.totalBruto)}
            />
            <KpiCard
              ...
              label="Lucro Líquido"
              value={formatarMoeda(kpis.totalLucro)}
            />
            <KpiCard
            ...
            <KpiCard
              ...
              label="Custo Total"
              value={formatarMoeda(kpis.totalCusto)}
            />
          </div>
        </div>
              ...
              >
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-400">
                    {v.bruto > 0 ? ((v.lucro / v.bruto) * 100).toFixed(1) : 0}%
                    margem
                  </p>
                </div>
              </div>
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.06)] border border-gray-100 dark:border-zinc-800 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 dark:border-zinc-800">
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Mostrando {dados.length} registro(s)
            </p>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50 dark:bg-zinc-800/50 border-b border-gray-100 dark:border-zinc-800">
                  ...
                  <th className="py-3 px-4 text-[10px] font-semibold text-gray-400 uppercase tracking-wider text-right">
                    Bruto
                  </th>
                  <th className="py-3 px-4 text-[10px] font-semibold text-gray-400 uppercase tracking-wider text-right">
                    Custo
