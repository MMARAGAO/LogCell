...
export function useTextoGarantiaPorTipo(tipoServico: TipoServicoGarantia | null) {
  const [textoGarantia, setTextoGarantia] = useState<TextoGarantia | null>(null);
  ...
  useEffect(() => {
    ...
    const buscarTexto = async () => {
      try {
        ...
        const { data, error: err } = await supabase
          .rpc('buscar_texto_garantia', { p_tipo_servico: tipoServico });
