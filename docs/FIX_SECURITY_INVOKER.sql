-- =====================================================
-- SOLUÇÃO: Mudar trigger para SECURITY INVOKER
-- =====================================================

-- O problema era SECURITY DEFINER com RLS para {authenticated}
-- SECURITY INVOKER usa as permissões do usuário que está fazendo a operação

CREATE OR REPLACE FUNCTION public.processar_baixa_estoque_os()
RETURNS trigger
LANGUAGE plpgsql
SECURITY INVOKER  -- <<<< MUDANÇA AQUI!
AS $function$
DECLARE
...
'Trigger BEFORE INSERT que baixa estoque. Usa SECURITY INVOKER para respeitar RLS do usuário.';

SELECT '✅ TRIGGER CORRIGIDO COM SECURITY INVOKER! Agora delete a peça e tente adicionar novamente.' as resultado;
