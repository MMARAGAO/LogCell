...
export function RecebimentoAparelhoModal({
  ...
  useEffect(() => {
    ...
    const forma =
      tipoPagamento === "cartao_credito" ? "cartao_credito" : "cartao_debito";
    ...
    try {
      ...
      if (editando && aparelho.venda_id) {
        ...
        for (const pagId of pagamentosRemovidos) {
          ...
          // Se for troca, remove também o aparelho do estoque
          if (removido?.tipo === "troca_aparelho" && aparelho.venda_id) {
            const trocaPattern = `%"venda_id":"${aparelho.venda_id}"%`;
          ...
          await supabase.from("historico_aparelhos").insert({
            ...
            descricao: `Pagamento${removido ? ` de ${formatarMoeda(removido.valor)} (${removido.tipo})` : ""} removido${removido?.tipo === "troca_aparelho" ? " e aparelho de troca removido do estoque" : ""}`,
            dados_antes: removido
              ? {
                  valor: removido.valor,
                  tipo: removido.tipo,
                  parcelas: removido.parcelas,
                }
              : null,
              ...
              <div className="flex gap-2 items-start flex-wrap">
                ...
                <Input
                  ...
                  onValueChange={(val) => {
                    ...
                    const formatted = (parseInt(digits) / 100).toLocaleString(
                      "pt-BR",
                      {
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs bg-gray-50 dark:bg-zinc-800 p-3 rounded-xl border border-gray-100 dark:border-zinc-700">
                  ...
                  {!taxaInclusa && infoTaxa.parcelas > 1 && (
                    <span className="text-gray-600 dark:text-gray-400">
                      Parcela:{" "}
                      <strong className="text-gray-800 dark:text-white">
                        {formatarMoeda(infoTaxa.bruto / infoTaxa.parcelas)} x{" "}
                        {infoTaxa.parcelas}
                      </strong>
                    </span>
                  )}
              ...
              {brindes.length > 0 && (
                <div className="space-y-1.5">
                  {brindes.map((b, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-2 bg-gray-50 dark:bg-zinc-800 p-2.5 rounded-xl border border-gray-100 dark:border-zinc-700"
                    >
              ...
              <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-[0_2px_12px_rgb(0,0,0,0.06)] border border-gray-100 dark:border-zinc-800 overflow-hidden">
                            ...
                            <div className="text-[11px] leading-tight space-y-0.5">
                              ...
                              {pag.liquido != null && (
                                <span className="text-emerald-600 dark:text-emerald-400">
                                  Receber: {formatarMoeda(pag.liquido)}
                                </span>
                              )}
                              {pag.tipo === "troca_aparelho" && (
                ...
                <div className="px-4 py-3 border-t border-gray-100 dark:border-zinc-800 space-y-1.5">
                  ...
                  <div className="flex justify-between text-xs pt-1.5 border-t border-gray-100 dark:border-zinc-800">
                    ...
                    >
                      {formatarMoeda(
                        totalPago - (aparelho.valor_compra || 0) - custoBrindes,
                      )}
                    </span>
                  </div>
                </div>
