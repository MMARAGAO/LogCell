...
CREATE TABLE public.ordem_servico (
  ...
  data_inicio_servico timestamp without time zone,
  data_conclusao timestamp without time zone,
  data_entrega_cliente timestamp without time zone,
  status character varying NOT NULL DEFAULT 'aguardando'::character varying,
...
CREATE TABLE public.ordem_servico_pagamentos (
  ...
  valor numeric NOT NULL CHECK (valor > 0::numeric),
  forma_pagamento character varying NOT NULL CHECK (forma_pagamento::text = ANY (ARRAY['dinheiro'::character varying, 'cartao_credito'::character varying, 'cartao_debito'::character varying, 'pix'::character varying, 'transferencia'::character varying, 'cheque'::character varying]::text[])),
  observacao text,
  criado_em timestamp without time zone DEFAULT now(),
  criado_por uuid,
...
CREATE TABLE public.ordem_servico_pecas (
  ...
  data_reserva_estoque timestamp without time zone,
  data_baixa_estoque timestamp without time zone,
  observacao text,
  criado_em timestamp without time zone DEFAULT now(),
  criado_por uuid,
...
CREATE TABLE public.vendas (
  ...
  status character varying NOT NULL CHECK (status::text = ANY (ARRAY['em_andamento'::character varying, 'concluida'::character varying, 'cancelada'::character varying]::text[])),
