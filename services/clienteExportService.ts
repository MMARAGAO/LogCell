...
const COLUNAS_MOEDA = new Set([
  "Total Gasto", "Total Pago", "Saldo Devedor", "Ticket Médio", "Créditos",
  "PIX", "Dinheiro", "Cartão de Crédito", "Cartão de Débito",
  "Transferência", "Boleto", "Credito Cliente", "Troca",
  "Valor Total Serviços", "Lucro Estimado",
]);
  ...
  for (const cliente of clientes) {
    try {
        ...
        r.totalVendas === 0
          ...
          : r.totalGasto >= 20000
            ...
            : (r.totalVendas >= 5 || r.totalAparelhos >= 5) &&
                (r.diasDesdeUltimaCompra == null || r.diasDesdeUltimaCompra < 60)
              ? "💎 Fiel"
              : r.diasDesdeUltimaCompra != null && r.diasDesdeUltimaCompra >= 90
                ? "💤 Perdido"
                : r.totalVendas === 1 && r.diasRelacionamento != null && r.diasRelacionamento < 30
                  ? "🆕 Novo"
                  : "👋 Regular";
      ...
      linhas.push({
        ...
        "Cliente Desde": cliente.criado_em ? formatarData(cliente.criado_em) : "—",
        ...
        "Primeira Compra": r.primeiraCompra ? formatarData(r.primeiraCompra) : "—",
