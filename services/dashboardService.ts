...
export class DashboardService {
  ...
  private static async buscarMetricasPeriodo(
    ...
    const { data: todasVendas } = await queryVendas;
  ...
  private static async buscarProdutosEstoqueBaixo(lojaId?: number) {
    ...
    return (
      estoques
        ?.filter(
          (e: any) =>
            e.produto && e.quantidade <= (e.produto.quantidade_minima || 0)
        )
        .map((e: any) => ({
          id: e.produto.id,
  ...
  private static async buscarMetricasAdicionais(
    ...
    const { data: contasReceber } = await queryContasReceber;
